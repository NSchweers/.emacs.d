...
** Set =auth-sources=
   #+BEGIN_SRC elisp
     (setf auth-sources
           (mapcar (lambda (dir)
                     (expand-file-name "authinfo.gpg" dir))
...
** Add todo states.
   ...
   #+END_SRC
** Enable blocking of dependencies
   #+BEGIN_SRC elisp
...
** Open menues for food venues in kemnat
   #+BEGIN_SRC elisp
     (defun schweers/food-kemnat ()
       (interactive)
       ;; (browse-url "http://z7-bar.de")
       (schweers/campioni '(4))
       (schweers/gourmet-compagnie))

     (defun schweers/campioni (&optional arg)
       (interactive "P")
       (when arg
         (other-window 1))
       (w3m "https://www.ristorante-campioni.de/index.php/karten/mittagskarte")
       (when arg
         (other-window -1)))

     (defun schweers/gourmet-compagnie (&optional arg)
       (interactive "P")
       (let*
           ((url-pattern
             "https://www.gourmet-compagnie.de/files/speiseplan_kw_%d_kemnat_%d.pdf")
            (current-count 0)
            (buffer nil)
            (buffer-name "*Gourmet Compagnie*"))
         (cl-labels ((next-url
                      ()
                      (format url-pattern
                              (car (read-from-string (format-time-string "%V")))
                              (incf current-count)))
                     (update-or-show-pdf
                      (status)
                      (if (plist-get status :error)
                        ...
                        (beginning-of-line)
                        (delete-region (point-min) (point))
                        (url-retrieve (next-url) #'update-or-show-pdf))))
           (url-retrieve
            (next-url) #'update-or-show-pdf))))
   #+END_SRC
